		...
		for(int i=0; i<2; i++)
		{
			if(pfds[i].revents & POLLIN)
			{
				if(i==0)
				{
					...
					sleep(1);
					nsfd[size]=socket(AF_INET, SOCK_STREAM, 0);
					struct sockaddr_in myaddr1;
					myaddr1.sin_family=AF_INET;
					myaddr1.sin_port=htons(porta);
					myaddr1.sin_addr.s_addr = inet_addr("127.0.0.1");
					
					int pp=connect(nsfd[size], (struct sockaddr*)&myaddr1, sizeof(myaddr1));
					if(pp<0)
						perror("connect");
