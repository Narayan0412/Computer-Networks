...
void *mncr(void *arg)
{
    ...
    //first msg to tell MNCR ur address
    char ch='a';
    sendto(sockfd, (const char *) &ch, sizeof(ch), MSG_CONFIRM, (const struct sockaddr *) &serverAddr, addrSize);

    //request for report
    char buf[100];
    recvfrom(sockfd, buf, sizeof(buf), 0, (struct sockaddr *) &serverAddr, &addrSize);
    
    //send number of cheated questions
    if(c==0)
        strcpy(buf, "0");           //no cheating
    else
        strcpy(buf, "1");

    sendto(sockfd, buf, sizeof(buf), 0, (const struct sockaddr *) &serverAddr, addrSize);
...
int main()
{
    ...
    //thread for mncr communication
    pthread_t mncr_thread;
    pthread_create(&mncr_thread, NULL, mncr, NULL);
    ...
    //set to NON_PROMISCUOUS_MODE if want to capture only packets sent to this machine
    pcap_t *handle = pcap_open_live(device.c_str(), BUFSIZ, NON_PROMISCUOUS_MODE, 1000, errorBuf);
    /*
