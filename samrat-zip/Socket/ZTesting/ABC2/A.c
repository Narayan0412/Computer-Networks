...
#define SERVER_PATH     "./UDS_B"
#define SERVER_PATH2    "./UDS_A"
...
int main()
{
	...
	unlink(SERVER_PATH2);
	int x=bind(usfd2, (struct sockaddr *)&myaddr1, sizeof(myaddr1));
	if(x<0)
		perror("bind");
	...
	while(1)
	{
		...
		printf("recieved fd\n");
		if(in.i==6)								//Termination, when all services are exhausted
			break;
	...
	send_fd(usfd, usfd, 6);						//Forward the termination
